...
from data import get_dil_dataloader
...
def load_til_trained_weights(
    ...
    if args.pretrained_weights:
        ...
        for i in range(args.num_tasks):
            model.add_task(task_id=i, n_classes=args.nb_classes)
...
def get_best_logits(logits_list: List[Any], total_classes: int) -> List:
    ...
    for batch_index, best_entropy_index in enumerate(best_entropy_indexes):
        required_logits.append(batch_task_logits[batch_index, best_entropy_index])
...
def main(args):
    ...
    print(
        f"************* Starting Evaluation for {args.num_tasks} tasks in DIL setup *************"
    )
    start_time = time.time()

    _, avg_stats = evaluate_dil(
        model=model,
        model_without_ddp=model_without_ddp,
