...
class Attention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.args = args
        print("In dim, Out dim, Num heads:", dim_in, dim_out, num_heads)
        if has_prefix:
            self.prefix = Prefix(
    ...
    def forward(self, x, h, w, prefix=True):
        if (
        ...
        **kwargs,
        ...
        if has_adapter:
            self.adapter = Adapter(
                embed_dim=dim_out,
                bottleneck=dim_out // args.reduction_factor,
                args=args,
            )

    def forward(self, x, h, w, prefix=True, adapter=True):
        res = x

        x = self.norm1(x)
        attn = self.attn(x, h, w, prefix=prefix)
        x = res + self.drop_path(attn)
        if self.has_adapter and adapter:
            r = self.adapter(self.mlp(self.norm2(x)))
        else:
            r = self.mlp(self.norm2(x))
        x = x + self.drop_path(r)
...
class VisionTransformer(nn.Module):
        ...
        for j in range(depth):
            blocks.append(
                Block(
                    ...
                    act_layer=act_layer,
                    norm_layer=norm_layer,
                    has_adapter=has_output_adapter and (j not in leave_out),
                    has_prefix=has_prefix and (j not in leave_out),
                    args=args,
                    **kwargs,
                )
            )
    ...
    def forward(self, x, prefix=True, adapter=True):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x, H, W, prefix=prefix, adapter=adapter)
...
class ConvolutionalVisionTransformer(nn.Module):
    def __init__(
        ...
        for i in range(self.num_stages):
            ...
            stage = VisionTransformer(
                ...
                act_layer=act_layer,
                norm_layer=norm_layer,
                has_output_adapter=has_output_adapter,
                has_prefix=has_prefix,
                leave_out=leave_out,
                args=args,
                **kwargs,
    ...
    def forward_features(self, x, prefix=True, adapter=True):
        for i in range(self.num_stages):
            x, cls_tokens = getattr(self, f"stage{i}")(
                x, prefix=prefix, adapter=adapter
            )
    ...
    def forward(self, x, pre_logits=False, prefix=True, adapter=True):
