...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None, prefix=True):
        """
        Args:
        ...
        if self.has_prefix and prefix:
            attn[..., self.prefix_length :] = (
                attn[..., self.prefix_length :] + self._get_rel_pos_bias()
            )
        else:
        ...
        if mask is not None:
            ...
            if self.has_prefix and prefix:
                temp = attn[..., self.prefix_length :]
                temp = temp.view(-1, num_win, self.num_heads, N, N) + mask.unsqueeze(
                    1
                ).unsqueeze(0)
                attn[..., self.prefix_length :] = temp.view(-1, self.num_heads, N, N)
            else:
...
class SwinTransformerBlock(nn.Module):
    ...
    def _attn(self, x, prefix=True):
        ...
        attn_windows = self.attn(
            x_windows, mask=self.attn_mask, prefix=prefix
        )  # nW*B, window_size*window_size, C
    ...
    def forward(self, x, prefix=True, adapter=True):
        B, H, W, C = x.shape
        x = x + self.drop_path1(self._attn(self.norm1(x), prefix=prefix))
        x = x.reshape(B, -1, C)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x, prefix=True, adapter=True):
        ...
        for layer in self.layers:
            x = layer(x, prefix=prefix, adapter=adapter)
        x = self.norm(x)
        return x
