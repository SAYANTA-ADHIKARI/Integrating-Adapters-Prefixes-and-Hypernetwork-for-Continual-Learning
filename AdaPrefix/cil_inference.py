...
def load_til_trained_weights(
    ...
    if args.pretrained_weights:
        ...
        peft_weights = checkpoint["peft"]
        return model, peft_weights
    else:
        raise ValueError(
        ...
        for input, target in metric_logger.log_every(
            ...
            for t in range(args.num_tasks):
                if args.has_atten_adapter or args.has_output_adapter or args.has_prefix:
                    model_without_ddp.load_peft(peft_weights, t)
                batch_logits.append(
...
def main(args):
    ...
    model = AdaPrefix(args)

    # Load the model weights
    model, peft_weights = load_til_trained_weights(model, args)
